    ...
    <section className="py-12 md:py-20 text-center">
      ...
      <div className="relative flex flex-col items-center gap-12 md:gap-16">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 md:left-1/2 h-full w-0.5 bg-border/20">
            <div className="h-full w-full bg-primary/80 animate-progress-bar origin-top" style={{animation: 'progress-bar 5s linear forwards', animationTimeline: 'view(y)'}}></div>
        </div>
        {steps.map((step, index) => (
          <div key={index} className="relative w-full flex flex-col items-center group">
            <div className="relative p-5 bg-card/80 backdrop-blur-sm rounded-full border-2 border-primary/20 z-10 group-hover:border-primary transition-colors duration-300 shadow-lg">
